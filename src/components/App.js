...
class App extends React.Component {
  ...
  setBearer = async (bearer, redirectTo = "/") => {
    ...
    let { decks } = await fetch("https://flipcards-server.herokuapp.com/user", {
      headers: {
  ...
  deleteDeck = deletedId => () => {
    this.setState(
      ({ decks }) => ({
        decks: decks.filter(({ id }) => id.toString() !== deletedId.toString()),
        redirectTo: "/"
      }),
      ...
      <>
        ...
        <Mobile>
          <Header
            ...
            redirectTo={location => this.setState({ redirectTo: location })}
          />
          <MobileSidebar
            decks={this.state.decks}
            updateDecks={this.updateDecks}
